 ...
 *
 * @author db117
 * @since 2021-07-26 17:47:21
 **/

public class Solution_1894 {
    public static void main(String[] args) {
        Solution solution = new Solution_1894().new Solution();
    ...
    class Solution {
        public int chalkReplacer(int[] chalk, int k) {
            ...
            int tmp = (int) (k % sums[sums.length - 1]);
            if (tmp == 0) {
                return 0;
            }

            // 查找的是 sums 索引（即 bs-1），但是需要找下一个(即 bs-1+1)
            return bs(sums, tmp);
        }

        // 查找小于等于目标值的最大值位置
        private int bs(long[] arr, int target) {
