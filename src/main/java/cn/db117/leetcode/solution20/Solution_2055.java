...
// 
// Related Topics 数组 字符串 二分查找 前缀和 👍 5 👎 0
 ...
 *
 * @author db117
 * @since 2021-11-03 18:14:31
 **/

public class Solution_2055 {
    public static void main(String[] args) {
        Solution solution = new Solution_2055().new Solution();
    ...
    class Solution {
        public int[] platesBetweenCandles(String s, int[][] queries) {
            ...
            for (int i = 0; i < queries.length; i++) {
                // 最左边的蜡烛
                Map.Entry<Integer, Integer> ceilingEntry = treeMap.ceilingEntry(queries[i][0]);
                // 最右边的蜡烛
                Map.Entry<Integer, Integer> floorEntry = treeMap.floorEntry(queries[i][1]);
                if (ceilingEntry != null && floorEntry != null) {
                    // 防止不在区间内
                    ans[i] = Math.max(0, floorEntry.getValue() - ceilingEntry.getValue());
                }
            }
            return ans;
        }
