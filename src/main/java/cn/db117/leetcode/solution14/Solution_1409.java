...
// 1 <= queries[i] <= m 
// 
// Related Topics 树状数组 数组 模拟 👍 30 👎 0
 ...
 *
 * @author db117
 * @since 2021-11-12 17:16:21
 **/

public class Solution_1409 {
    public static void main(String[] args) {
        Solution solution = new Solution_1409().new Solution();
    ...
    class Solution {
        public int[] processQueries(int[] queries, int m) {
            // 模拟
            List<Integer> list = IntStream.range(1, m + 1).boxed().collect(Collectors.toList());
            int[] ans = new int[queries.length];

            for (int i = 0; i < queries.length; i++) {
                int indexOf = list.indexOf(queries[i]);
                ans[i] = indexOf;
                list.remove(indexOf);
